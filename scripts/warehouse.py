...
JOIN dim_movies m ON cm.movie_id = m.movie_id
JOIN dim_genres g ON g.genre = ANY(string_to_array(cm.genres, '|'));
""")
...
CREATE INDEX idx_bridge_genre_key ON bridge_movie_genres(genre_key);
""")

# execute warehouse table creation
logging.info("Starting warehouse creation.")

try:
    with engine.begin() as conn:
        logging.info("Dropping existing warehouse tables if any exists")
        ...
        logging.info("Creating bridge_movies_genres...")
